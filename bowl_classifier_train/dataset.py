...
class KaggleDataset(Dataset):
    ...
    def __init__(self, root, transform=None, cache=None, category=None, source=None, img_folder=None,resize_scale=[512,512]):
        ...
        if os.path.isfile(root+'.csv'):
            ...
            self.label    = [self.majorlabels[x] if x in self.majorlabels.keys() else 'None' for x in list(df['major_category'])]
            self.sublabel = [self.sublabels[x] if x in self.sublabels.keys() else 0 for x in list(df['sub_category'])]
    ...
    def split(self,K_fold=None,K_idx=None):
        ...
        random.setstate(s)
        # if K_fold == n, --> loocv
        split        = int(np.floor( 1 / K_fold * n))
        kfold_beg    = K_idx * split
...
if __name__ == '__main__':
    ...
    dataset = KaggleDataset('./stage1_train_fix_v4_external',
                            transform=Compose(), 
                            img_folder='./bowl_stage1_data/stage1_train_fix_v4_external',
                            category=None,
                            resize_scale=[128,128])
    train_idx, valid_idx = dataset.split(K_fold=3,K_idx=0)
    print('dataset len = ' + str(dataset.__len__()))
    print('train_idx len ' + str(len(train_idx)))
    print('valid_idx len ' + str(len(valid_idx)))
    ...
    print(dataset.sublabel)
    '''  
    print('{:<3s} {:<15s} {:^5s} {:^8s} {:<65s}'.format('id','source','label','sublabel', 'uid'))  
    count = 0
    for i, data in enumerate(train_loader, 0):
        ...
        for j in range(len(inputs)):
            if (sources[j] == 'Kaggle') & (labels[j].cpu().numpy()==2) :
                print('{:<3d} {:<15s} {:^5d} {:^8d} {:<65s}'.format(i,sources[j], labels[j].cpu().numpy(), sublabels[j].cpu().numpy(),uids[j]))
                count += 1       
