...
def main(tocsv=False, save=False, mask=False, valid_train=False, toiou=False, submit_folder=False):
    ...
    data_dir = 'data/stage1_train' if valid_train else config['param']['CSV_PATH']
    print(data_dir)
    data_dir = re.sub('\.csv','',data_dir)
    dataset = KaggleDataset(data_dir, transform=compose, img_folder=config['param']['img_folder'])
    ...
    if valid_train:
        ...
        if not os.path.exists(data_dir):
            print('%s does not exist.  It will not generate %s/iou_valid.csv\nBye bye!'%(data_dir, submit_folder))
        else:
...
def predict(model, dataset, compose, resize):
    ...
    for data in dataset:
        ...
        # gt, gt_s, gt_c, gt_m = filter_fiber(gt), filter_fiber(gt_s), filter_fiber(gt_c), filter_fiber(gt_m)
        ...
        # y, y_c, y_m = filter_fiber(y), filter_fiber(y_c), filter_fiber(y_m)
...
if __name__ == '__main__':
    ...
    parser.set_defaults(csv=True, save=False, mask=False, dataset='test', iou=False)
    ...
    main(args.csv, args.save, args.mask, args.dataset == 'train', args.iou, submit_folder)
