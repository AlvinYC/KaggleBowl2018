...
def make_windows(f, scale, bases, stride):
    windows  = []
    _, _, H, W = f.size()
    for y, x in itertools.product(range(math.ceil(H/stride)),range(math.ceil(W/stride))):
        cx = x*scale*stride
        cy = y*scale*stride
        for b in bases:
            x0,y0,x1,y1 = b
...
def make_rpn_windows(cfg, fs):
    ...
    for l in range(num_scales):
        ...
        # print ('bases', bases)
        windows = make_windows(fs[l], cfg.rpn_scales[l], bases, cfg.rpn_stride)
        # print ('windows', windows.shape)
        rpn_windows.append(windows)

    rpn_windows = np.vstack(rpn_windows)
    # print ('rpn_windows',rpn_windows)
    return rpn_windows
