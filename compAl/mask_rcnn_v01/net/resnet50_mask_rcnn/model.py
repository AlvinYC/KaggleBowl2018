...
def make_layer_c0(in_planes, out_planes, stride):
    layers = [
        nn.Conv2d(in_planes, out_planes, kernel_size=7, stride=stride, padding=3, bias=False),
...
class FeatureNet(nn.Module):
    ...
    def __init__(self, cfg, in_channels, out_channels=256 ):
        ...
        self.cfg=cfg
        self.layer_maxpool = nn.MaxPool2d( 2)
        if cfg.high_resolution == True:
            self.layer_c0 = make_layer_c0(in_channels, 64, cfg.layer0_stride)
            self.layer_c1 = make_layer_c(   64,  32,   64, num_blocks=3, stride=1)  #out =  64*4 =  256
            self.layer_c2 = make_layer_c(   64,  32,  128, num_blocks=4, stride=2)  #out = 128*4 =  512
            self.layer_c3 = make_layer_c(  128,  64,  256, num_blocks=6, stride=2)  #out = 256*4 = 1024
            self.layer_c4 = make_layer_c(  256, 128,  512, num_blocks=3, stride=2)  #out = 512*4 = 2048
            # top-down
            
            self.layer_p4 = nn.Conv2d   (  512, out_channels, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x):
        ...
        c2 = self.layer_c2(c1)       #; 
        # print('layer_c2 ',c2.size())
        c3 = self.layer_c3(c2)       #; 
        # print('layer_c3 ',c3.size())
        c4 = self.layer_c4(c3)       #; 
        # print('layer_c4 ',c4.size())


        p4 = self.layer_p4(c4)       #; 
        # print('layer_p4 ',p4.size())
        p3 = self.layer_p3(c3, p4)   #; 
        # print('layer_p3 ',p3.size())
        p2 = self.layer_p2(c2, p3)   #; 
        # print('layer_p2 ',p2.size())
        p1 = self.layer_p1(c1, p2)   #; 
        # print('layer_p1 ',p1.size())
        p5 = self.layer_maxpool(p4)
        # print('layer_p5 ',p5.size())
        if self.cfg.rpn_p0_pool == True:
            r2 = self.layer_maxpool(p1)
            # print('layer_r2 ',r2.size())
            r3 = self.layer_maxpool(r2)
            # print('layer_r3 ',r3.size())
            r4 = self.layer_maxpool(r3)
            # print('layer_r4 ',r4.size())
            r5 = self.layer_maxpool(r4)
            # print('layer_r5 ',r5.size())
            features_rpn = [p1,r2,r3,r4,r5]
        else:    
            features_rpn = [p1,p2,p3,p4,p5]
        features_rcnn = [p1,p2,p3,p4,p5]
        assert(len(self.cfg.rpn_scales) == len(features_rpn))

        return features_rpn, features_rcnn
...
class RpnMultiHead(nn.Module):
    ...
    def __init__(self, cfg, in_channels):
        ...
        self.pool = nn.MaxPool3d((3,1,1), (3,1,1))
        self.rpn_stride = cfg.rpn_stride
        channels = in_channels*2
        if cfg.rpn_p0_pool == True:
            self.conv1   =  nn.Conv2d(in_channels, channels, kernel_size=3, stride=self.rpn_stride,  padding=1)
            self.logits1 = nn.Conv2d(channels, self.num_bases[1]*self.num_classes, kernel_size=3, padding=1)
            self.deltas1 = nn.Conv2d(channels, self.num_bases[1]*self.num_classes*4, kernel_size=3, padding=1)
        for l in range(self.num_scales):
            self.convs.append ( nn.Conv2d(in_channels, channels, kernel_size=3, stride=self.rpn_stride,  padding=1) )
    ...
    def forward(self, fs):
        ...
        for l in range(self.num_scales):  # apply multibox head to feature maps
            ...
            # print ('convs', self.convs[l])

            f = F.dropout(f, p=0.5,training=self.training)
            # print ('after convs', f.size())
            if self.cfg.rpn_p0_pool==True: 
                logit = self.logits1(f)
                delta = self.deltas1(f) 
                # print ('logit size before', logit.size())
                # print ('delta size before', delta.size())
                if l == 0:
                    logit = self.pool(logit) 
                    delta = self.pool(delta)
                    # print ('logit size after', logit.size())
                    # print ('delta size after', delta.size())
            else:
                logit = self.logits[l](f)
                delta = self.deltas[l](f)

            logit_flat = logit.permute(0, 2, 3, 1).contiguous().view(batch_size, -1, self.num_classes)
            # print ('logit shape', logit_flat.shape)
            delta_flat = delta.permute(0, 2, 3, 1).contiguous().view(batch_size, -1, self.num_classes, 4)
            # print ('delta_flat shape', delta_flat.shape)
            prob_flat  = F.sigmoid(logit_flat)
            # print ('prob_flat shape', prob_flat.shape)
...
class CropRoi_1layer(nn.Module):
    ...
    def forward(self, fs, proposals):
        ...
        rois = Variable(rois)
        crops = self.crops(fs[0], rois.view(-1,5))
...
class MaskRcnnNet(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if cfg.crop_one_layer == True:
            self.rcnn_crop   = CropRoi_1layer  (cfg, cfg.rcnn_crop_size)
            self.mask_crop   = CropRoi_1layer  (cfg, cfg.mask_crop_size)
        else:
    ...
    def forward(self, inputs, truth_boxes=None,  truth_labels=None, truth_instances=None ):
        ...
        #rpn proposals -------------------------------------------
        self.rpn_logits_flat, self.rpn_probs_flat, self.rpn_deltas_flat = data_parallel(self.rpn_head, features_rpn)
        ...
        if len(self.detections)>0:
              ...
              self.masks, self.keeps, self.category_labels, self.label_sorted = mask_nms(cfg, mode, inputs, self.detections, self.mask_logits) #<todo> better nms for mask
