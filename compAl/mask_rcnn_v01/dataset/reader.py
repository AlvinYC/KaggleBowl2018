...
class ScienceDataset(Dataset):
    ...
    def __init__(self, comment_csv, csv_path=None, transform=None, mode='train'):
        super(ScienceDataset, self).__init__()
        start = timer()
        self.label_column = config['train'].get('label_column')
        self.classes_map = json.loads(config['maskrcnn'].get('classes_map'))
        self.comment_csv = comment_csv
        self.transform = transform
        self.mode = mode 
        self.comment_path = csv_path

        #read split
        ids = comment_csv['image_id'].drop_duplicates().values.tolist()
    ...
    def __getitem__(self, index):
        ...
        # name   = id.split('/')[-1]
        # folder = id.split('/')[0]
        #path_image = glob.glob(os.path.join(DATA_DIR + '/image/', '*', 'images/', id +'.png'))
        #path_mask = glob.glob(os.path.join(DATA_DIR + '/image/', '*', 'multi_masks/', id +'.npy'))
        path_image = glob.glob(os.path.join(DATA_DIR + '/', '*', 'images/', id +'.png'))
        path_mask = glob.glob(os.path.join(DATA_DIR + '/', '*', 'multi_masks/', id +'.npy'))
        
        #print ('path_image', path_image)
        image = cv2.imread( path_image[0], cv2.IMREAD_COLOR)
        # image = cv2.imread(DATA_DIR + '/image/%s/images/%s.png'%(folder,name), cv2.IMREAD_COLOR)
        
        # print (category)
        # print ("csv_label", label)

        if self.mode in ['train']:
            category = self.comment_csv[self.label_column][index]
            cat_id = self.classes_map[category]
            # print ("csv_label", label)
            multi_mask = np.load( path_mask[0]).astype(np.int32)
            meta = '<not_used>'

            if self.transform is not None:
                return self.transform(image, multi_mask, meta, index,  cat_id)
            else:
                return input, multi_mask, meta, index

        if self.mode in ['test']:
            if self.transform is not None:
                return self.transform(image, index, id)
            else:
                return image, index
...
def multi_mask_to_contour_overlay(cfg, multi_mask, detection, keep,image=None, color=[255,255,255]):
    pre_score_threshold = cfg.mask_test_nms_pre_score_threshold
    height,width = multi_mask.shape[:2]
    overlay = np.zeros((height,width,3),np.uint8) if image is None else image.copy()
    num_masks = int(multi_mask.max())
    if num_masks==0: return overlay
    # index = np.where(detection[:,5]>pre_score_threshold)
    # detection = detection[index]
    for i in range(num_masks):
        keep_index = keep[i]
        det = detection[keep_index]
        s = det[5]
        b = det[1:5]
        label_str = str(int(det[6]))
        # color2 = to_color(s, [255,0,255])
        # cv2.rectangle(overlay,(b[0], b[1]), (b[2], b[3]), color2, 1)
        font = cv2.FONT_HERSHEY_SIMPLEX
        # prob_round = round(det[5], 3)
        # print ('label_str', label_str)
        color_map = {'1' : (255,0,0), '2' : (0,255,0), '3':  (0,0,255), '4': (125,0,125),
                     '5' : (125,125,0), '6' : (0,125,125), '7': (195, 65, 195) }
        mask = multi_mask==i+1
        contour = mask_to_inner_contour(mask)
        overlay[contour]=color_map[label_str]
        x_center = int((b[2]+b[0])/2)
        y_center = int((b[3]+b[1])/2)
        # cv2.putText(overlay, str(prob_round), (b[2],b[3]), font, .3, (0, 255, 0), 1, 2)
        cv2.putText(overlay,label_str, (x_center,y_center), font, .3, color_map[label_str], 1, 2)

    return overlay

def multi_mask_to_contour_overlay_no_detection(multi_mask, image=None, color=[255,255,255]):
...
def multi_mask_to_annotation(multi_mask, one_label):
    ...
    for i in range(num_masks):
        ...
        if mask.sum()>1:
            ...
            #label
            l = one_label #<todo> support multiclass later ... ?
